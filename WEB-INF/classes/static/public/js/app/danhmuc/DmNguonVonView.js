var DmNguonVonView = function(){
	...
	this.oDmNguonVon = new DmNguonVon();
	...
	this.search = function(){
		that.oDmNguonVon.search('');
		that.oTable.clear().draw();
        var aRows = [];
		for (var i = 0; i < that.oDmNguonVon.LIST.length; i++) {
			var item = that.oDmNguonVon.LIST[i];
			var _hidden = '<input type="hidden" class="rowID" value="' + item.nguonvonid + '" />';
	...
	this.bindForm = function(){
		$('#ma').val(that.oDmNguonVon.ma);
		$('#ten').val(that.oDmNguonVon.ten);
		$('#ghichu').val(that.oDmNguonVon.ghichu);
		$('#tongiatri').val(that.oDmNguonVon.tongiatri);
		$('#trangthai').val(that.oDmNguonVon.trangthai);
	}

	this.clearForm = function(){
		that.oDmNguonVon.nguonvonid = 0;
		$('#ma').val('');
		$('#ten').val('');
		$('#ghichu').val('');
		$('#tongiatri').val(0);
	...
	this.save = function(){
		...
		that.oDmNguonVon.ma = $('#ma').val();
		that.oDmNguonVon.ten = $('#ten').val();
		that.oDmNguonVon.ghichu = $('#ghichu').val();
		that.oDmNguonVon.tongiatri = $('#tongiatri').val();
		that.oDmNguonVon.trangthai = $('#trangthai').val();

		var strAlert = that.oDmNguonVon.validSave();
		if (!strAlert == '') {
			oAlert.show(strAlert, '40%', '50px');
			return false;
		}

		var rs = that.oDmNguonVon.save();
		if (rs.CODE == 0) {
		...
		$('.ACTIONS').on('click', '#btnSave', function () {
		 ...
		 $('#Grid01 tbody').on('click', 'tr', function () {
            if ($(this).hasClass('selected')) {
				...
				that.oDmNguonVon.nguonvonid = id;
				that.oDmNguonVon.getById();
				that.bindForm();
		   }
		   return true;
		 });
		 ...
		 $('.ACTIONS').on('click', '#btnDelete', function () {
			...
			var oAlert = new AlertDialog('Thông báo');
			if (that.oDmNguonVon.nguonvonid == 0) {
				oAlert.show('Bạn chưa chọn mục cần xóa', '40%', '50px');
				return false;
			}

			function ok() {
				var rs = that.oDmNguonVon.del();
