var DmTienTeView = function(){
	...
	this.search = function(){
		that.oDmTienTe.search('');
		that.oTable.clear().draw();
        var aRows = [];
		for (var i = 0; i < that.oDmTienTe.LIST.length; i++) {
			var item = that.oDmTienTe.LIST[i];
			var _hidden = '<input type="hidden" class="rowID" value="' + item.tienteid + '" />';
	...
	this.bindForm = function(){
		$('#ma').val(that.oDmTienTe.ma);
		$('#ten').val(that.oDmTienTe.ten);
		$('#ghichu').val(that.oDmTienTe.ghichu);
		$('#tygia').val(that.oDmTienTe.tygia);
		$('#trangthai').val(that.oDmTienTe.trangthai);
	}

	this.clearForm = function(){
		that.oDmTienTe.tienteid = 0;
		$('#ma').val('');
		$('#ten').val('');
		$('#ghichu').val('');
		$('#tygia').val(0);
	...
	this.save = function(){
		...
		that.oDmTienTe.ma = $('#ma').val();
		that.oDmTienTe.ten = $('#ten').val();
		that.oDmTienTe.ghichu = $('#ghichu').val();
		that.oDmTienTe.tygia = $('#tygia').val();
		that.oDmTienTe.trangthai = $('#trangthai').val();

		var strAlert = that.oDmTienTe.validSave();
		if (!strAlert == '') {
			oAlert.show(strAlert, '40%', '50px');
			return false;
		}

		var rs = that.oDmTienTe.save();
		if (rs.CODE == 0) {
		...
		$('.ACTIONS').on('click', '#btnSave', function () {
		 ...
		 $('#Grid01 tbody').on('click', 'tr', function () {
            if ($(this).hasClass('selected')) {
				...
				that.oDmTienTe.tienteid = id;
				that.oDmTienTe.getById();
				that.bindForm();
		   }
		   return true;
		 });
		 ...
		 $('.ACTIONS').on('click', '#btnDelete', function () {
			...
			var oAlert = new AlertDialog('Thông báo');
			if (that.oDmTienTe.tienteid == 0) {
				oAlert.show('Bạn chưa chọn mục cần xóa', '40%', '50px');
				return false;
			}

			function ok() {
				var rs = that.oDmTienTe.del();
